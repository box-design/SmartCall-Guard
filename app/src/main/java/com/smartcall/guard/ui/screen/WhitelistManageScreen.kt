...
@Composable
fun WhitelistManageScreen(
    viewModel: WhitelistManageViewModel = hiltViewModel(),
    onBack: () -> Unit
) {
        ...
        Column(
            ...
            if (filteredRules.isEmpty()) {
                Column(
                    ...
                    Text("暂无白名单规则", style = MaterialTheme.typography.bodyLarge)
                }
            } else {
                LazyColumn(
    ...
    if (showAddDialog) {
        AddWhitelistRuleDialog(
            onDismiss = { showAddDialog = false },
            onSave = { value, note, tag ->
                viewModel.addWhitelistRule(value, note, tag)
                showAddDialog = false
            }
        )
    }
...
fun AddWhitelistRuleDialog(
    ...
    androidx.compose.material3.ModalBottomSheet(
        ...
        Column(
            ...
            Text("添加白名单号码", style = MaterialTheme.typography.titleLarge)
            ...
            FilledTonalButton(
                onClick = {
                    if (value.isNotBlank()) {
                        onSave(value, note.ifBlank { null }, tag.ifBlank { null })
                    }
                },
                modifier = Modifier.fillMaxWidth(),
